...
import { Plus, X, Trash2, Save, UserPlus, Loader2 } from "lucide-react"
...
export function AddPatientForm({ onPatientAdded, onCancel }: { onPatientAdded?: (patient: Patient) => void; onCancel?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || user.role !== "doctor") {
      setError("Only doctors can add patients")
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(`Patient added successfully! Patient ID: ${result.patient.pid}`)
      } else {
        throw new Error(result.error || 'Failed to add patient')
      }
